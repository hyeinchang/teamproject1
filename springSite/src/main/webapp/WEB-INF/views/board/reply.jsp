...
<script type="text/javascript">
	...
	$(function(){
		...
		$(document).on("click",".update_form", function() {
			...
			if (replyPwdConfirm == 0) {
				replyNum = currLi.attr("data-num");
				btnKind = "upBtn";
				pwdView(currLi);
			} else if (replyPwdConfirm == 1) {
		...
		$(document).on("click", ".pwdCheckBut", function() {
			...
			if (!formCheck(pwd, r_msg, "비밀번호를")) return;
				...
				$.ajax({
					url : "/replies/replyPwdConfirm.do",
					type : "post",
	...
	function pwdView(area) {
		...
		data += "<input type='passwd' name='passwd' class='passwd'>";
