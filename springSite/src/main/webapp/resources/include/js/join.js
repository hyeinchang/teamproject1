...
$(function(){
	...
	var message = ["영문, 숫자만 가능. 6 ~ 12자로 입력해주세요,",
		"영문, 숫자, 특수문자만 가능. 8 ~ 15자 입력해주세요.",
		"비밀번호와 비밀번호 확인란은 값이 일치해야합니다.",
		"- 포함 입력해주세요. 예시)010-0000-0000"];
	...
	$("#idConfirmBtn").click(function() {
		if (!formCheck($('#userId'), $('.error:eq(0)'),"아이디를")) return;
			...
			$.ajax({
				...
				error : function() {
					alert("사이트 접속에 문재로 정상 작동하지 못하였습니다. 다시 시도해 주세요.");
				},
	...
	$("#joinInsert").click(function() {
		...
		else if (idConfirm != 2) {alert("아이디 중복 체크를 실행해주세요."); return;}
		else {
